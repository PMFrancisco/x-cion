  ...
  return useMutation({
    mutationFn: async ({ postId, isLiked }: { postId: string; isLiked: boolean }) => {
      ...
      if (isLiked) {
        ...
        const { error } = await supabase
          .from("likes")
          .insert({ user_id: effectiveProfileId, post_id: postId });
        if (error) throw error;
      }
    },
    ...
    onMutate: async ({ postId, isLiked }) => {
      // Cancelar refetches en curso para que no sobreescriban nuestro cambio optimista
      ...
      queryClient.setQueriesData<PostWithCounts>({ queryKey: ["post", postId] }, (old) => {
        if (!old) return old;
        return { ...old, is_liked: !isLiked, like_count: old.like_count + (isLiked ? -1 : 1) };
      });
    ...
    onError: (_err, { postId }, context) => {
      ...
      // Re-sincronizar la vista de hilo
